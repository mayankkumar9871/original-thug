...
# ✅ FIXED: tries in IND, BR, EUROPE order
async def detect_player_region(uid: str):
    preferred_order = ["IND", "BR", "EUROPE"]
    for region_key in preferred_order:
        server_url = _SERVERS.get(region_key)
        tokens = _token_cache.get_tokens(region_key)
        if not server_url or not tokens:
            continue
        try:
            ...
            logger.error(f"Region {region_key} detection failed: {str(e)}")
    return None, None
